...
echo "This is a Customized version of CKEditor's build.sh. Any updates to the original build.sh has to be updated here. ..."
...
case $1 in
	...
	*)
		...
		echo "Usage:"
		echo "$0 -v"
		echo "$0 basic|standard|svmx|full [all] [-t]"
		echo ""
		exit 1
		;;
esac
...
SVMX_BUILD_PATH=build/$versionFolder/svmx/ckeditor
STATIC_RESOURCE_ZIP_FILE_NAME="svmx_ckeditor.zip"
echo ""
echo "Build created into the \"build\" directory."

echo "Copying Build to Designer 2.0 Public folder. ..."
echo "It is recommended that the projects are in same parent directory."
echo "If any path adjustments to be done please update build.sh file. ..."
echo "Cleaning Copy Target folder"
(rm -rf ../designer-2.0/public/ckeditor)
echo "Removing samples folder from build"
(rm -rf $SVMX_BUILD_PATH/samples)
(rm -rf $SVMX_BUILD_PATH/.github)
echo "Copying build ..."
(cp -R  $SVMX_BUILD_PATH ../designer-2.0/public/)
echo "Build Complete ..."
echo ""
